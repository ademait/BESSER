...
def test_agentic_lane_swarm_size_default():
    """Constructed without the kwarg -> swarm_size is 1 (single agent)."""
    lane = AgenticLane(name="Coder")
    assert lane.swarm_size == 1


def test_agentic_lane_swarm_size_set():
    lane = AgenticLane(name="Coder", swarm_size=3)
    assert lane.swarm_size == 3


@pytest.mark.parametrize("value", [0, -1])
def test_agentic_lane_swarm_size_out_of_range(value):
    with pytest.raises(ValueError, match=r"swarm_size must be >= 1"):
        AgenticLane(name="Coder", swarm_size=value)


@pytest.mark.parametrize("value", [True, 2.0])
def test_agentic_lane_swarm_size_type_error(value):
    with pytest.raises(TypeError, match="swarm_size must be an int"):
        AgenticLane(name="Coder", swarm_size=value)


def test_agentic_lane_repr_includes_swarm_size():
    lane = AgenticLane(name="Coder", swarm_size=3)
